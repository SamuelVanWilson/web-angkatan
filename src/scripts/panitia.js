export async function initPanitiaPage() {
  ...
  function createCardHTML(item) {
    ...
    const CardAttr = instagramHandle ? `href="https://www.instagram.com/${instagramHandle}" target="_blank" rel="noopener noreferrer"` : '';

    return `
            <${CardTag} ${CardAttr} class="block shrink-0 w-[280px] snap-center group relative bg-white rounded-3xl overflow-hidden hover:shadow-xl transition-all duration-300 border border-gray-100 hover:-translate-y-2">
                <div class="aspect-square bg-gray-100 relative overflow-hidden">
                    ${imageHTML}
                </div>
                <div class="p-6 relative flex flex-col justify-between h-[120px]">
                    <div>
                    ...
                    ${instagramHandle ? `
                    <div class="flex items-center gap-1.5 text-gray-400 group-hover:text-pink-500 transition-colors duration-300">
                        ...
                        <span class="text-xs font-semibold truncate">@${instagramHandle}</span>
                        <svg class="w-4 h-4 ml-0.5 text-gray-300 group-hover:text-pink-500 transform group-hover:translate-x-1 transition-all duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
